...
import queue
import random
from machine import SoftI2C, SoftSPI, Pin, I2C, SPI
import struct
import utime
import time
...
class NRF24L01:
    ...
    def express_recv(self):
        self.cs(0)
        self.spi.readinto(self.buf, FIFO_STATUS)
        self.spi.readinto(self.buf)
        self.cs(1)
        if not self.buf[0] & RX_EMPTY:
...
nrf.fast_send(struct.pack("i", start_packet))

for i in range(total_packets):
    time.sleep(0.001)
    nrf.fast_send(struct.pack("iff", i, 3.1415, -6.283))

for i in range(1000):
    nrf.fast_send(struct.pack("i", end_packet))
...
#     if nrf.any():
#
#         a = struct.unpack("iff", nrf.recv())[0]
#
#         if a == start_packet:
...
#         counter = counter + 1
#
# dt = time.ticks_diff(end_time, start_time) / 1000000
# freq = counter / dt
# lost_ratio = (total_packets - counter) / total_packets
# print(f'counter = {counter}, dt = {dt}, freq: {freq}, lost_ratio: {lost_ratio}')
#
# print('over')
...
# while True:
#     nrf.cs(0)
#     nrf.spi.readinto(nrf.buf, FIFO_STATUS)
#     nrf.spi.readinto(nrf.buf)
#     nrf.cs(1)
#     if not nrf.buf[0] & RX_EMPTY:
#         # there is something to read
#         # get the data
#         nrf.cs(0)
#         nrf.spi.readinto(nrf.buf, R_RX_PAYLOAD)
#         buf = nrf.spi.read(nrf.payload_size)
#         nrf.cs(1)
#         # clear RX ready flag
#         nrf.reg_write(STATUS, RX_DR)
#
#         a = struct.unpack("iff", buf)
#         print(a)
#         a = a[0]
#
#         if a == start_packet:
