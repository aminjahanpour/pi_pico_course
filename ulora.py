...
class ModemConfig():
    Bw125Cr45Sf128 = (0x72, 0x74, 0x04)  # < Bw = 125 kHz,      Cr = 4/5, Sf = 128 chips/symbol, CRC on. Default medium range
    Bw500Cr45Sf128 = (0x92, 0x74, 0x04)  # < Bw = 500 kHz,      Cr = 4/5, Sf = 128 chips/symbol, CRC on. Fast+short range
    Bw31_25Cr48Sf512 = (0x48, 0x94, 0x04)  # < Bw = 31.25 kHz,  Cr = 4/8, Sf = 512 chips/symbol, CRC on. Slow+long range
    Bw125Cr48Sf4096 = (0x78, 0xc4, 0x0c)  # /< Bw = 125 kHz,    Cr = 4/8, Sf = 4096 chips/symbol, low data rate, CRC on. Slow+long range
    Bw125Cr45Sf2048 = (0x72, 0xb4, 0x04)  # < Bw = 125 kHz,     Cr = 4/5, Sf = 2048 chips/symbol, CRC on. Slow+long range

    stm32 = (0x72, 0xb4, 0x04)  # < Bw = 125 kHz,     Cr = 4/5, Sf = 2048 chips/symbol, CRC on. Slow+long range
...
class LoRa(object):
    def __init__(self,
        ...
        assert self._spi_read(REG_01_OP_MODE) == (MODE_SLEEP | LONG_RANGE_MODE), "LoRa initialization failed"
...
# initialise radio
def create_lora(this_address, mc='pico'):
    ...
    if mc == 'pico':
        ...
        spi_bus = SPIConfig.esp32_heltec_v2

        reset_pin = 14
        cs_pin = 18
        interrupt_pin = 26

    lora = LoRa(this_address=this_address,
